...
If the observer agent fails (timeout, spawn error, invalid output), the system SHALL silently drop the observation. No raw text fallback is stored. This matches claude-mem's behavior.
...
- **WHEN** config has `observerModel: "litellm-bedrock/haiku-latest"`
- **THEN** the observer uses that model for extraction

#### Scenario: Fallback to summary model
- **WHEN** config has no `observerModel` but has `summaryModel: "litellm-bedrock/haiku-latest"`
